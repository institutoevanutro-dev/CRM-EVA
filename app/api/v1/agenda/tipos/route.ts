...
const camposDoTipo = {
  ...
  required_room_kind: z.enum(["consultation", "application"]).nullish(),
  concurrency_key: z.string().trim().min(1).max(80).nullish(),
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  return ok(
    r.tipos.map((t) => ({
      ...
      catalog_product_id: t.produtoCatalogoId,
      required_room_kind: t.tipoDeSalaNecessaria,
      concurrency_key: t.chaveDeSimultaneidade,
